...
1. Your task is to extract behavioral specifications from the source codebase. For each source module, produce a spec at `specs/{module_name}.md` describing WHAT the code does (not HOW). Every behavioral claim MUST cite the source: `[source:path/to/file:line-range]`. Use up to 500 parallel Sonnet subagents for reading source files. Use an Opus subagent to synthesize findings into specs. Ultrathink.
2. For test files, extract the behavioral truth they encode. For source files, extract public API contracts, error handling, side effects, and state management. Cross-reference test specs with source specs.

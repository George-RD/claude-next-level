...
description: |
  ...
  user: "Check parity for the auth module: spec at specs/from-src/auth.spec.md, target at src-ts/auth.ts"
...
- **Source spec path** — the behavioral spec extracted from the original source module (`specs/from-src/*.spec.md`)
- **Test spec path** (optional) — the behavioral spec extracted from the original test files (`specs/from-tests/*.spec.md`)

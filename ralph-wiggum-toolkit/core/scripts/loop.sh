...
if [[ ! -f "$PROMPT_FILE" ]]; then
    ...
    echo "Run /ralph init first, or create it manually."
    exit 1
fi
...
if [[ ! -d "specs" ]] || [[ -z "$(ls -A specs/ 2>/dev/null)" ]]; then
    ...
    echo "Ralph works best with specs. Run /ralph spec first."
    echo ""
fi

# Read model from manifest if present (shell-only, no python3 dependency)
CLAUDE_MODEL="opus"
if [[ -f "ralph/manifest.json" ]]; then
  MANIFEST_MODEL=$(grep '"default_model"' ralph/manifest.json | sed 's/.*": *"\([^"]*\)".*/\1/' 2>/dev/null || echo "opus")
